	...
	private async started() {
		...
		buttonBehavior.onHover("enter", () => {
			...
			MRE.Animation.AnimateTo(this.context, this.cube, {
				destination: {
					transform: {
						local: { scale: { x: 0.35, y: 0.35, z: 0.35 } },
					},
				},
				duration: 0.3,
		...
		buttonBehavior.onHover("exit", () => {
			MRE.Animation.AnimateTo(this.context, this.cube, {
				destination: {
					transform: { local: { scale: { x: 0.3, y: 0.3, z: 0.3 } } },
				},
				duration: 0.3,
				easing: MRE.AnimationEaseCurves.EaseOutSine,
			});
		...
		*/
		...
		buttonBehavior.onClick((user) => {
			...
			this.wearFakeHat(user.id);
		});
	}

	private wearFakeHat(userId: MRE.Guid) {
		...
		this.attachedHats.set(userId, this.soundActor);
